...
Given our analytical objectives, we subset the shot-level dataset to include only approach shots.  The PGA Tour defines[^1] "approach shot" (for statistics purposes) as a shot that does not originate from on or around the green, and ends up on or around the green or in the hole.  The Tour further defines "on or around the green" as being within 30 yards of the edge of the green.  Because our dataset does not indicate distance from the edge of the green, but rather distance from the pin, we will use a 35-yard distance from the hole in determining "on or around the green". Applying this threshold results in a dataset of `r nrow(approachShots)` approach shots taken in the sampled 14 tournaments.

[^1]: http://www.pgatour.com/stats/stat.02325.html

## Course Feature Data

To examine the impact of course features on approach shot performance, we use the `golfcoursegeo` package[^2] to obtain, for each hole:
...
Our analysis will consider the following as explanatory (or independent) variables for these outcomes:
...
As mentioned above, we initially included weather data in the analysis, but found that none of the explanatory variables included (air temperature, wind speed, and barometric pressure) were significant in explaining the dependent variables.  Thus, these variables will not be discussed further here.
...
In summary:  nothing really unexpected here.  Horsey hits most of his approach shots from the fairway, though three times he went for the green on par four holes, as well (every tee shot on a par three is considered an approach shot).  He is getting onto the green the vast majority of the time with his approach shots, which is also to be expected from a professional golfer.

### Relationships of Key Variables

The regression and random forest models that we will fit later will suggest significant relationships between the dependent variables (`DistanceToPinAfter` and `ShotResult`) and the length of the approach shot (`DistanceToPinBefore`) and the lie of the ball for the approach shot (`ShotLie`).  But we can begin to get a sense of these relationships in a simple table:
...
We can examine these same relationships visually:
...
In the next phase of our analysis, we attempt to model these relationships more rigorously, and also include course features in the model to determine the impacts of things like green area and greenside hazards.

# Regression and Classification Models

Recall from our prior discussion that we have identified two approach shot outcomes of interest:  having the ball come to rest as close to the hole as possible, and having the ball come to rest on the green.  We have performed a preliminary analysis of two factors that seem to influence these outcomes:  the length of the approach shot, and the lie of the ball for the approach shot.  It would appear, from tabular and graphical analysis, that these explanatory variables are correlated with the outcomes; now we will estimate a least-squares regression for the `DistanceToPinAfter` dependent variable, a logistic regression as well as a random forest classification model for the `ShotResult` dependent variable.
...
Through experimentation, we determined that the counts of greenside hazards and bunkers were not significant explanatory factors for either of our outcome (dependent) variables.  We had thought _a priori_ that Horsey might tend to play approach shots in a manner that avoids greenside hazards, but there really is no evidence that this is the case.  Similarly, we had expected that the par for the hole would be a significant explanatory factor, but this also turned out not to be the case--suggesting that, in fact, approach shots taken as tee shots on par threes are not significantly different from approach shots on par fours and fives. In what follows, we will only present models fitted without these variables included.

### Least Squares Regression: Explaining Variation in Proximity to the Hole

To analyze factors that explain the distance remaining to the hole after the approach shot, we fit the following ordinary least squares regression:
$$
...
$$
Note that the $\boldsymbol\beta_2$ represents a vector of coefficients on dummy variables representing the type of lie, with fairway lies held out as the reference value.

We obtain the following coefficients, standard errors, and p-values from fitting this regression to the `r nrow(approachShotsWithCourseInfo)` approach shot observations:
...
The available data do not allow us to test these hypotheses, but doing so would be a valuable area of further research.
...
Now we turn our attention to explaining a different outcome; rather than analyzing proximity to the hole following an approach shot, we will look at whether Horsey succeeded at having the ball come to rest on the green.  In many ways, this is a more valuable insight, since most golfers would rather have a 25-foot putt for birdie (having hit the green in regulation) than a bunker shot 20 feet from a tucked pin, needing to get up-and-down for par. We will use logistic regression to _classify_ each of Horsey's `r nrow(approachShotsWithCourseInfo)` approach shots as successful or not, in terms of the ball resting on the green after the shot.

Whereas least squares regression seeks to explain variation in a variable, like `DistanceToPinAfter`, that is continuous over a range of values, logistic regression explains variation in a discrete variable.  A common scenario is to use logistic regression to explain a binary outcome, like `ShotResult`.  The regression coefficients (indirectly) allow us to estimate the impact of the dependent or explanatory variables on the odds of one outcome occurring versus the other.

We fit the same explanatory variables as in the least squares regression model above, but now the dependent variable is the log-odds ratio of being on the green versus not:
...
As with the least squares regression analysis above, we see that `DistanceToPinBefore` (i.e., the length of the shot), and the lie being in the Fairway Bunker or Rough are significant factors in explaining the odds of Horsey hitting the green with his approach shot.  However, we have a new significant explanatory variable in this analysis:  area of the green.  This makes sense, because the area of the green has a direct impact upon whether a ball that is hit to within a specified distance of the hole is in fact on the green.  This means, of course, that if Horsey and his caddie were primarily interested in hitting the green on a given hole--versus just trying to get it as close to the pin as possible--then this analysis enables them to factor in the size of the green in choosing among their tactical options.

It is easiest to interpret the effects of the factors by considering the marginal odds ratio effects.  For example, holding all other factors constant, a one foot increase in the length of the approach shot (`DistanceToPinBefore`) reduces the odds of hitting the green by `r format(100*(1 - (logitDf %>% filter(Variable=='DistanceToPinBefore') %>% .$OR)), digits=3)` percent.  Similarly, though somewhat more alarmingly, hitting an approach shot from the rough reduces the odds of coming to rest on the green by `r format(100*(1 - (logitDf %>% filter(Variable=='ShotLie: Rough') %>% .$OR)), digits=3)` percent.
...
ggplot(data=df) + geom_line(aes(x=Distance, y=Odds, color=Lie)) + theme_economist() + theme(legend.position='right', legend.text=element_text(size=9))
```

Of course, it would be reasonable to ask whether this model, overall, is better than nothing at predicting success at green-hitting.  There are R-squared like measures for logistic regression, but the more common measure of model quality is the Likelihood Ratio test.  It compares the likelihood of observing the estimated model to the likelihood of the "null hypothesis" model that includes only the intercept term.  The test for the model we have fitted here gives a p-value of zero, meaning we can reject the null hypothesis.

Another measure of goodness-of-fit for a logistic regression (or any classification model) is to look at the _confusion matrix_, which displays the number of true and false positives and negatives predicted by the model.  The confusion matrix for our logistic regression model is:
...
In quantifying the specific marginal effects of factors like length of shot, lie, and course characteristics for a specific player (in our case, European Tour member David Horsey), we have shown how relatively simple statistical modeling techniques can help pinpoint expectations and, hopefully, help a player and his or her caddie make better course management decisions.

The analysis here also suggests potentially fruitful topics of further research.  First and foremost, it would be instructive to compare the model coefficients for David Horsey to his fellow competitors, both on the European Tour and the PGA Tour.  It would also be helpful to collect more observations of intermediate rough and tee lies, to see if a larger sample would boost the significance of those variables.  We were disappointed that weather data proved insignificant in the models fitted here; perhaps more frequent observations, that introduced greater variance, would open up possibilities there.
